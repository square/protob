...
expandPath = function(str) {
  var pth = str.replace(/\$\w+/g, function(envVar) { return process.env[envVar.replace('$', '')]; });
...
function Source(sourceDef){
  ...
  if ( sourceDef.git ) {
    ...
    this.basePath = expandPath(sourceDef.local);
  } else {
   ...
   */
  this.fetch = function(){
    if ( this.fetched || this.local ) { return Promise.resolve().then(function() { return true; }); }
...
Source.prototype.fetchFromGit = function(){
  ...
  return new Promise(function (resolve, reject) {
    if ( fs.existsSync(self.basePath) ) {
      ...
      console.error("Cloning from git:", self.git);
      // clone it and check out the branch
      var cmd = "git clone --branch " + self.branch + " " + self.git;
...
Source.prototype.fetchFromNexus = function(){
  if(!this.nexus) return new Promise(function(res, rej) { res() });
  var self = this;

  return new Promise(function (resolve, reject) {
    var fileName = self.basePath + ".zip",
        start = Date.now(),
        opts = require('url').parse(self.url.format());

    console.log("Fetching from nexus:", self.url.format());

    require('rimraf').sync(self.basePath);
    ...
    require('https').get(opts, function(res){
      ...
      res.on('error',  function(err){ console.log("Response error:", err); reject(err) });
      fileStream.on('error', function(err){ console.log("Response error:", err); reject(err) });

      fileStream.on('finish', function(){
        var theFile = Path.basename(fileName),
            thePath = Path.basename(self.basePath);

        exec("mkdir -p " + thePath + "; unzip " + theFile + " -d " + thePath, {cwd: Protofile.protoCache}, function(err, stdout, stderr){
          require('fs').unlink(theFile, function(){});
          if(err){
            console.error(err);
            reject(err);
            return;
