...
var INTEGER32 = function(fieldName, signed) {
  ...
  var func = function(val) {
    ...
    if ( result > maxSize ) {
      ...
      throw(fieldName + ": is too small at " + result + ". Should be less than " + minSize);
    }
    return result;
  };
  return func;
...
var INTEGER64 = function(fieldName, signed) {
  ...
  var func = function(val) {
    ...
    if ( value.greaterThan(maxSize) ) {
      throw(fieldName + " is too large", value.toString() );
    } else if ( value.lessThan(minSize) ) {
      throw(fieldName + " is too small", value.toString() );
    }
    return value;
  };
  return func;
